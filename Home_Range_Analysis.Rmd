...
<details><summary><big>Importing dataset with ```read.csv```:</big></summary>
...
raster2 <- openmap(c(36.440453, -87.309515),c(36.436534, -87.319491), type="bing")
...
I also changed the point and polygon colors to red in order to see them better on the dark background (especially since I couldn't get it to zoom in any closer to the points).
...
mcp_raster <- function(filename){
  ...
  mcp.plot <- autoplot.OpenStreetMap(raster2_utm, expand = TRUE) + theme_bw() + theme(legend.position="none") +
    theme(panel.border = element_rect(colour = "black", fill=NA, size=1)) +
    geom_polygon(data=mcp.poly, aes(x=mcp.poly$long, y=mcp.poly$lat), color="red",fill="#993333", alpha=0.8) +
    geom_point(data=mcp.points, aes(x=x, y=y), color = "red") + 
...
gridded(xy) <- TRUE
class(xy)
#x <- seq(xmin,xmax,by=resolution) # where resolution is the pixel size you desire 
#y <- seq(ymin,ymax,by=resolution)
kde=kernelUD(data[,1],h=h, grid=xy, kern=c("bivnorm"))
  data.proj <- SpatialPointsDataFrame(xy,data, proj4string = CRS("+proj=utm +zone=16 +datum=WGS84 +units=m +no_defs"))
#  xy <- SpatialPoints(data.proj@coords)
#  kde<-kernelUD(xy, h="href", kern="bivnorm", grid=95)
  ver <- getverticeshr(kde, 95)
  kde.points <- cbind((data.frame(data.proj@coords)),data$individual.local.identifier)
  colnames(kde.points) <- c("x","y","identifier")
  kde.poly <- fortify(ver, region = "id")
